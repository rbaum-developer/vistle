...
int ReadRST::OpenFile(const std::string &filename)
{
    ...
    // Reading the first 4 bytes of the binary file, i.e. the file number
    size_t iret = fread(buf.get(), sizeof(int), 1, rfp_);

    if (iret != 1) {
        std::cerr << "Error reading rfp_" << std::endl;
    }
    ...
    // Reading the header
    if (fread(buf.get() + 1 + header_offset, sizeof(int), 100, rfp_) != 100) {
        return (2);
    }
    ...
    int offset_head = 2;
    header_.filenum_ = SwitchEndian(buf[1 + offset_head]);
    header_.format_ = SwitchEndian(buf[2 + offset_head]);
    header_.time_ = SwitchEndian(buf[3 + offset_head]);
    header_.date_ = SwitchEndian(buf[4 + offset_head]);
    header_.unit_ = SwitchEndian(buf[5 + offset_head]);
    header_.ansysdate_ = SwitchEndian(buf[11 + offset_head]);
    memcpy(header_.machine_, &buf[12 + offset_head], 3 * sizeof(int));
    header_.machine_[12] = '\0';
    SwitchEndian(header_.machine_, 12);
    memcpy(header_.jobname_, &buf[31 + offset_head], 8 * sizeof(int));
    header_.jobname_[8] = '\0';
    SwitchEndian(header_.jobname_, 8);
    memcpy(header_.product_, &buf[17 + offset_head], 2 * sizeof(int));
    header_.product_[8] = '\0';
    SwitchEndian(header_.product_, 8);
    memcpy(header_.label_, &buf[19 + offset_head], 1 * sizeof(int));
    header_.label_[4] = '\0';
    SwitchEndian(header_.label_, 4);
    memcpy(header_.user_, &buf[20 + offset_head], 3 * sizeof(int));
    header_.user_[12] = '\0';
    SwitchEndian(header_.user_, 12);
    memcpy(header_.machine2_, &buf[23 + offset_head], 3 * sizeof(int));
    header_.machine2_[12] = '\0';
    SwitchEndian(header_.machine2_, 12);
    header_.recordsize_ = SwitchEndian(buf[26 + offset_head]);
    header_.maxfilelen_ = SwitchEndian(buf[27 + offset_head]);
    header_.maxrecnum_ = SwitchEndian(buf[28 + offset_head]);
    memcpy(header_.title_, &buf[41 + offset_head], 20 * sizeof(int));
    header_.title_[80] = '\0';
    SwitchEndian(header_.title_, 80);
    memcpy(header_.subtitle_, &buf[61 + offset_head], 20 * sizeof(int));
    ...
    std::cout << "raw buf[1+offset_head]=" << buf[1 + offset_head] << " switched=" << SwitchEndian(buf[1 + offset_head])
              << std::endl;

    //TODO: check what number means here - is 12 in the covise version
    // fun12 - unit number (resu file is 12)
    int isResultFile = 12;
    if (SwitchEndian(buf[1 + offset_head]) != isResultFile) {
        ChangeSwitch();
        if (SwitchEndian(buf[1 + offset_head]) == isResultFile) {
            ...
            std::cerr << "Cannot correctly read file header length" << std::endl;
            return 2;
        }
    }
