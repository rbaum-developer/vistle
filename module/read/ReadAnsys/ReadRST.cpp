...
int ReadRST::OpenFile(const std::string &filename)
{
    ...
    if (get_file_size() != 0) {
        Vistle::sendError("Could not get file size");
        return (1);
    }
    ...
    sendInfo("File generated by ANSYS V%d.%d", header_.version_, subversion);
    ...
    sendInfo("Machine: %s   Jobname: %s", header_.machine_, header_.jobname_);
    sendInfo("Product: %s   Label: %s", header_.product_, header_.label_);
    sendInfo("Title: %s   Subtitle: %s", header_.title_, header_.subtitle_);
    ...
    if (SwitchEndian(buf[2]) != 12) {
        ...
        if (SwitchEndian(buf[2]) == 12) {
            ...
            fclose(rfp_);
            // try again
            sendInfo("File is byteswapped, trying again ...");
            return OpenFile(filename);
        } else {
            // file cannot be read at all
            sendError("Cannot correctly read file header length");
            return 2;
        }
    }
...
int ReadRST::Read(const std::string &filename, int num, std::vector<int> &codes)
{
    ...
    case 1:
        sendError("GetData : file not open\n");
        break;

    case 2:
        sendError("GetData : read error: DSI\n");
        break;

    case 3:
        sendError("GetData : num exeeds limits\n");
        break;

    case 4:
        sendError("GetData : read error solution header\n");
        break;

    case 5:
        sendError("GetData : read error DOFs\n");
        break;

    case 6:
        sendError("GetData : read error exDOF\n");
        break;
    }
    if (problems)
        return problems;
    ...
    case 1:
        sendError("GetNodes : read error geo\n");
        break;

    case 2:
        sendError("GetNodes : read error nodes\n");
        break;

    case 3:
        sendError("GetNodes : read error element description\n");
        break;

    case 4:
        sendError("GetNodes : read error ety\n");
        break;

    case 5:
        sendError("GetNodes : read error element tabular\n");
        break;

    case 6:
        sendError("GetNodes : read error elements\n");
        break;
    }
    //  fclose(rfp);
    //  rfp=NULL;
   ...
   */
    ...
    for (i = 0; i < codes.size(); ++i) {
        ...
        if (codes[i] < ReadANSYS::EX_OFFSET) // non-extra normal scalar DOF
        {
            ...
            if (code_order == solheader_.numdofs_) {
                sendError("The code of a degree of freedom was not found");
                return 5;
            }
...
int ReadRST::GetNodes(void)
{
    ...
    // Some statistics
    sendInfo("Total number of nodes: %d", ghdr.nodes_);
    sendInfo("Total number of elements: %d", ghdr.elements_);
    ...
    for (i = 0; i < ANSYS::LIB_SIZE; ++i) {
        if (populations[i]) {
            ...
            sendInfo("Element type [%d]: %d elements", i, populations[i]);
        }
    }
